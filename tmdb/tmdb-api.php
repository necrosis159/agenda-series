...
class TMDB{
	 ...
	 */
	public function __construct($apikey, $lang = 'fr', $debug = false) {
		// Sets the API key
	...
	public function getKeyword($idTVShow, $page = 1) {
		$keywords = array();

		$result = $this->_call('keyword/' . $idTVShow, 'page='. $page);
	...
	public function getPopularTVShow($page = 1) {
		$show = array();

		$result = $this->_call('tv/popular','page='. $page);

		foreach($result['results'] as $data){
			$show[] = new TVShow($data);
		}

		return $show;
	}

	public function getTopRatedTVShow($page = 1) {
		$show = array();
	...
	public function getChangedTVShow($id, $page = 1) {

		$result = $this->_call('tv/' . $id . '/changes', 'page='. $page);

		return $result;
	}

	public function getChangedSeason($id, $page = 1) {

		$result = $this->_call('tv/season/' . $id . '/changes', 'page='. $page);

		return $result;
	}

	public function getChangedEpisode($id, $page = 1) {

		$result = $this->_call('tv/episode/' . $id . '/changes', 'page='. $page);
