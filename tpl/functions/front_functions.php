...
/*LES SERIES*/
  ...
  return $query;
   ...
   function series_get_active_season($idSerie){
    $db = call_pdo();
    $seriesDetail = $db->query('SELECT * FROM season WHERE id='.$idSerie.' AND status="1"');
    return $seriesDetail;
   }
  ...
  function add_comment($idUser, $idEpisode, $title, $date, $comment, $notation, $status){
      ...
      $db->query("INSERT INTO `agendaserieovh`.`comment` (`id`, `id_episode`, `id_user`, `date_publication`, `title`, `content`, `notation`, `status`) VALUES (NULL, '".$idEpisode."', '".$idUser."', '2015-02-04', 'azdazdazd', '".$comment."', '5', '1');");
  }
